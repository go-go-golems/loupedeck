...
GOLANGCI_LINT_ARGS ?= --timeout=5m ./cmd/... ./pkg/... ./runtime/... ./examples/cmd/... ./dev-tools/...
...
release:
	...
	GOWORK=off GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/loupedeck@$(shell svu current)
...
install:
	GOWORK=off go build -o ./dist/loupedeck ./cmd/loupedeck && \
		cp ./dist/loupedeck $(LOUPEDECK_BINARY)
...
logcopter-check:
	GOWORK=off go tool logcopter-gen -area-prefix go-go-golems.loupedeck -strip-prefix github.com/go-go-golems/loupedeck -check ./cmd/... ./pkg/... ./runtime/...
