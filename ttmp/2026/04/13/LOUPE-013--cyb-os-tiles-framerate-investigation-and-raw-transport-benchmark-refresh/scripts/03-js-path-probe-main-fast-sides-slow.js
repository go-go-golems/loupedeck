...
function drawMain() {
  main.batch(() => {
    ...
    for (let y = 0; y < MAIN_H; y += 18) {
      main.fillRect(0, y, MAIN_W, 1, 10);
    }
    for (let x = 0; x < MAIN_W; x += 24) {
      main.fillRect(x, 0, 1, MAIN_H, 8);
    }
...
function drawLeft() {
  left.batch(() => {
    left.clear(0);
    for (let y = 0; y < SIDE_H; y += 20) {
      left.fillRect(0, y, SIDE_W, 1, 12);
    }
    const slowFrame = (frame / SIDE_DIVISOR) | 0;
    const y = (slowFrame * 7) % (SIDE_H + 20) - 20;
...
function drawRight() {
  right.batch(() => {
    right.clear(0);
    for (let y = 10; y < SIDE_H; y += 24) {
      right.fillRect(0, y, SIDE_W, 1, 12);
    }
    const slowFrame = (frame / SIDE_DIVISOR) | 0;
    const y = (slowFrame * 5 + 40) % (SIDE_H + 20) - 20;
    right.fillRect(0, y, SIDE_W, 20, 140);
  });
}

ui.page("js-path-probe-main-fast-sides-slow", page => {
...
ui.show("js-path-probe-main-fast-sides-slow");
present.onFrame(() => {
  drawMain();
  if (frame % SIDE_DIVISOR === 0) {
    drawLeft();
    drawRight();
  }
});
